 ...
 *
 * $FreeBSD: stable/9/contrib/top/top.c 239750 2012-08-27 19:55:19Z jhb $
 */
...
cprintf(const char * font, const char * __restrict __format, args) {
    ...
    char clr[lenghtclr];
    bzero(clr, sizeof(font));
    strcpy(clr, font);
    char clrcl[] = "\033[m";


	linesize=sizeof(__format);
	//char frm[linesize];
	char frm[100];
	...
	formatsize=sizeof(clr)+sizeof(clrcl);
	len = strlen(frm);

	if (len > sizeof(frm) - formatsize)
		len = sizeof(frm) - formatsize;
	memmove(frm + sizeof(clr)-1, frm, len);
	for (i=0; i<(sizeof(clr)-1);i++)
	{
...
#ifdef ORDER
    static char command_chars[] = "\f qh?en#sdkriIutHmSCajzPoc";
#else
    static char command_chars[] = "\f qh?en#sdkriIutHmSCajzPc";
#endif
...
#define CMD_color       25
	...
	while ((i = getopt(ac, av, "CSIHPabijnquvzs:d:U:m:o:t")) != EOF)
	{
		...
		fprintf(stderr,
"Top version %s\n"
"Usage: %s [-abcCHIijnPqStuvz] [-d count] [-m io | cpu] [-o field] [-s time]\n"
"       [-U username] [number]\n",
