 ...
 *
 * $FreeBSD: stable/9/usr.bin/top/machine.c 239750 2012-08-27 19:55:19Z jhb $
 */
...
static char up_header[] =
    "  PID%s %-*.*s "   "PRI NICE   SIZE    RES STATE    TIME %6s   TSWAP COMMAND";
 ...
 */
char *ordernames[] = {
	"cpu", "size", "res", "swsz", "time", "pri", "threads", "tswap", 
...
#define ORDERKEY_SWSIZE(a, b) do { \
	long diff = (long)(b)->ki_swrss - (long)(a)->ki_swrss; \
	if (diff != 0) \
		return (diff > 0 ? 1 : -1); \
} while (0)
...
#define ORDERKEY_TSWAP(a, b) do { \
	int diff = (int)(b)->ki_swtime - (int)(a)->ki_swtime; \
	if (diff != 0) \
		return (diff > 0 ? 1 : -1); \
} while (0)
...
int compare_size(), compare_res(), compare_swsz(), compare_time(), compare_prio(),
    compare_threads(), compare_tswap();
...
/* compare_swsz - the comparison function for sorting by resident set size */

int
compare_swsz(void *arg1, void *arg2)
{
	struct kinfo_proc *p1 = *(struct kinfo_proc **)arg1;
	struct kinfo_proc *p2 = *(struct kinfo_proc **)arg2;
...
/* compare_threads - the comparison function for sorting by threads */
int
compare_tswap(void *arg1, void *arg2)
{
	struct kinfo_proc *p1 = *(struct kinfo_proc **)arg1;
	struct kinfo_proc *p2 = *(struct kinfo_proc **)arg2;

	ORDERKEY_TSWAP(p2, p1);
	ORDERKEY_JID(p1, p2);
	ORDERKEY_PCTCPU(p1, p2);
	ORDERKEY_CPTICKS(p1, p2);
	ORDERKEY_STATE(p1, p2);
